...
function resolveBranchProofs(node: MerkelNode, proof: Uint8Array = new Uint8Array(), depth = 0): Proof | Proof[] {
  ...
  if (node.type == "branch") {
    ...
    return [resolveBranchProofs(node.leftChild!, partialProof, depth + 1), resolveBranchProofs(node.rightChild!, partialProof, depth + 1)] as [
      Proof,
      Proof,
    ];
  }
...
export const arrayCompare = (a: Uint8Array | any[], b: Uint8Array | any[]) => a.every((value: any, index: any) => b[index] === value);
