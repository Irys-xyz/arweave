 ...
 * @param config
 */
export async function* downloadTx(txId: string, api: Api | FallbackApi, options?: { concurrency?: number }): AsyncGenerator<Buffer> {
  // default concurrency to 100 as the chunks are *tiny* (256kib)
  const opts = { concurrency: 100, ...options };
  try {
    const metadata = await api.get<{ offset: number; size: number }>(`/tx/${txId}/offset`);
    ...
    const getChunk = async (offset: BigNumber): Promise<Buffer> =>
      api.get<{ chunk: string }>(`/chunk/${offset.toString()}`).then((r) => {
