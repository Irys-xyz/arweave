...
import { pemTojwk, jwkTopem } from "../common/lib/crypto/pem";
...
export class NodeCryptoDriver implements CryptoInterface {
  ...
  public generateJWK(): Promise<JWKInterface> {
    ...
    return new Promise((resolve, reject) => {
      crypto.generateKeyPair(
        ...
        (err: Error | null, _publicKey: string, privateKey: string) => {
          if (err) {
            reject(err);
          }
  ...
  public sign(jwk: object, data: Uint8Array, { saltLength }: SignatureOptions = {}): Promise<Uint8Array> {
