...
describe("Chunks", function () {
  ...
  it("should validate own proofs and reject invalid verification parameters", async function () {
    ...
    const didValidateWithInvalidInputA = await validatePath.apply(validatePath, invalidInputA);
    ...
    const didValidateWithInvalidInputB = await validatePath.apply(validatePath, invalidInputB);
  ...
  it("should validate a valid data path against a valid data root", async function () {
    expect(await validatePath(root, offset, 0, dataSize, path)).toBeTruthy();
  });

  it("should reject invalid root", async function () {
    const invalidRoot = b64UrlToBuffer("lX5K7gAUlIMt2hYYkoXVrjmVMnnjF6P6c5sov6mPqCm");
    expect(await validatePath(invalidRoot, offset, 0, dataSize, path)).toBe(false);
  });
  ...
  it("should reject invalid path", async function () {
    ...
    expect(await validatePath(root, offset, 0, dataSize, invalidPath)).toBe(false);
  });
